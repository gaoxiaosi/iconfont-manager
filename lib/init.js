...
async function init (phoneNumber, password) {
  ...
  const filePath = path.join(homedir, '.iconfontrc');
  !fs.existsSync(filePath) && await fs.createFile(filePath)
  await fs.writeJSON(filePath, { projects });
  console.log(`
  初始化完毕，你可以${chalk.green('iconfont-update-cli ls')}查看你的所有项目.
  请设置好图标的保存目录(绝对路径)再进行其他操作，设置方法如下：
  1.在${chalk.green(homedir)}目录下找到${chalk.green('.iconfontrc')}文件直接编辑;
  2.执行${chalk.green('iconfont-update-cli ui')}命令通过图形化界面进行配置.`)
}

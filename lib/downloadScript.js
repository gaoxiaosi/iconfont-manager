...
// 解压downloa.zip
function compressingZip(savePath) {
  compressing.zip.uncompress(joinPath(savePath, 'download.zip'), savePath)
    .then(() => deleteDir(savePath))
    .catch(err => console.error(err))
}

// 删除iconfont文件夹(如果存在）和下载的download.zip
// 注意：需等文件夹删除完毕再重名
...
// download.zip解压后前缀为font_的文件夹重命名为iconfont
 ...
 * @param {Boolean} isRelogin 是否重新登录
...
const downloadScript = async (id, name, user, password, filePath, isRelogin, isCloseBrowser) => {
  ...
  if (isNeedLogin) {
    ...
    // 等待页面所有请求结束后（理论上登录成功，跳转完页面了，如果页面上还有提交按钮，说明登录失败了）
    await page.waitForNetworkIdle();
    const submitBtn = await page.$('.mx-btn-submit')
    if (submitBtn !== null) {
      spinner.fail(chalk.red('登录失败！账号或密码错误或其他原因，请重新确认再登录！'));
      return
    }
  ...
  // 登录成功后，打开项目链接
  spinner.start(chalk.green('跳转到图标库管理页'));
  await page.goto(`${projectLibraryUrl}&projectId=${id}`, {
    waitUntil: 'networkidle0'
  })
  spinner.succeed(chalk.green('图标库管理页跳转成功'));
  ...
  // 下载超时或某些情况,文件夹下可能有download.zip和download.zip.crdownload，需要先删除再下载
  ...
  spinner.start(chalk.green('开始下载图标'));
  ...
  spinner.succeed(chalk.green('图标下载完成'));
  if (isRelogin) {
    spinner.start(chalk.green('开始退出登录'));
    ...
    await page.waitForNetworkIdle();
    spinner.succeed(chalk.green('退出登录成功'));
    isNeedLogin = true
  }
  if (isCloseBrowser) {
    await page.close();
    log(chalk.green('✔ 关闭Page'));
    await browser.close();
    log(chalk.green(('✔ 关闭Browser')));
  ...
  // 解压、删除、重命名
  compressingZip(savePath)
  log(chalk.green((`✔ 图标库:${name} 更新完成🎉🎉🎉`)));
}
